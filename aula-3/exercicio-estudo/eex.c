...
/* =========================================================================
 * FUNÇÕES BÁSICAS FORNECIDAS / REAPROVEITADAS DO EXERCÍCIO ANTERIOR
 * ========================================================================= */

/**
 * Lê um registro delimitado precedido por seu tamanho (indicador de tamanho).
 * Formato em disco: [tamanho (int: 4 bytes)] [dados do registro (tamanho bytes)]
 */
int pega_registro(FILE *p_out, char *p_reg)
{
    ...
    else
    {
        ...
        p_reg[bytes] = '\0'; /* Garante terminação nula em memória */
        return bytes;
    }
}

/**
 * Lê o próximo campo delimitado por '|' a partir da posição *p_pos.
 */
...
/**
 * Grava um registro no arquivo binário precedido pelo seu tamanho em bytes.
 */
...
/**
 * Extrai apenas o CPF (primeiro campo) de um registro serializado.
 */
...
/**
 * Imprime um registro delimitado de forma formatada para o usuário.
 */
...
/**
 * Monta a string do registro delimitada por '|'.
 * Formato: cpf|nome|sobrenome|telefone|cidade|
 */
...
/* =========================================================================
 * REQUISITO 1: INSERÇÃO ORDENADA (INTERCALAÇÃO VIA ARQUIVO TEMPORÁRIO)
 * ========================================================================= */
int inserir_ordenado(const char *nome_arquivo, const char *novo_reg)
{
    ...
    /* Caso 1: Arquivo ainda não existe, cria diretamente */
    if (f_in == NULL)
    {
    ...
    /* Caso 2: Arquivo já existe. Intercala usando arquivo temporário */
    ...
    /* Se o novo registro for maior que todos os existentes no arquivo */
    if (!inserido)
    {
        grava_registro(f_temp, novo_reg);
    }
...
/* =========================================================================
 * REQUISITO 2: REMOÇÃO FÍSICA COM REORDENAÇÃO (VIA ARQUIVO TEMPORÁRIO)
 * ========================================================================= */
int remover_fisico(const char *nome_arquivo, const char *cpf_alvo)
{
    ...
    while (pega_registro(f_in, reg_atual) > 0)
    {
        ...
        {
            removido = 1; /* Pula a gravação deste registro (remoção física) */
        }
        else
        {
            grava_registro(f_temp, reg_atual);
...
/* =========================================================================
 * REQUISITO 3: ATUALIZAÇÃO (UPDATE) PRESERVANDO ORDEM (VIA TEMPORÁRIO)
 * ========================================================================= */
...
/* =========================================================================
 * REQUISITO 4 & PEGADINHA PROPOSITAL: BUSCA BINÁRIA ADAPTADA POR ÍNDICE
 * =========================================================================
 *
 * RESPOSTA À PEGADINHA PROPOSITAL (DOCUMENTAÇÃO CONCEITUAL):
 *
 * Problema: Registros de tamanho variável não possuem endereço aritmético direto
 * (não é possível calcular byte_offset = meio * sizeof(Registro)). Por isso,
 * criamos um vetor em RAM com os offsets iniciais de cada registro no arquivo.
 *
 * O que acontece se o arquivo for modificado (inserção/remoção/atualização)
 * após a geração do índice?
 * -> Os offsets armazenados no vetor de índices tornam-se INVÁLIDOS (stale offsets).
 *    Uma inserção ou remoção desloca fisicamente todos os bytes subsequentes.
 *    Um fseek baseado em offset antigo apontará para o meio de um campo ou
 *    para o registro errado! O índice é, estritamente, uma "FOTO" (snapshot)
 *    do arquivo naquele instante de tempo.
 *
 * TRADE-OFFS DAS DUAS ABORDAGENS:
 *
 * 1) Recalcular o índice a cada chamada de busca:
 *    - Vantagem: Consistência garantida (nunca acessa dados inválidos/corrompidos).
 *    - Desvantagem: Custo assintótico degradado. Montar o índice requer varrer o
 *      arquivo sequencialmente de ponta a ponta (O(N) I/Os de disco). Fazer O(N)
 *      para depois fazer busca binária O(log N) anula o ganho da busca binária,
 *      sendo até pior que uma busca sequencial direta que para ao encontrar.
 *
 * 2) Manter o índice em memória e passá-lo pronto como parâmetro:
 *    - Vantagem: Busca binária legítima em O(log N) acessos aleatórios via fseek.
 *      Altíssimo desempenho em cargas de trabalho com muitas consultas (read-heavy).
 *    - Desvantagem: Necessidade de sincronização/invalidação. Qualquer operação
 *      de inserção, remoção ou atualização exige reconstruir ou atualizar o índice.
 *
 * Abaixo implementamos a montagem explícita do índice e a busca binária sobre ele.
 * ========================================================================= */

/**
 * Percorre o arquivo sequencialmente uma vez e constrói o vetor dinâmico de offsets.
 */
...
/**
 * Executa a busca binária utilizando o array de offsets e saltos diretos via fseek.
 */
                             ...
                             const char *cpf_busca, char *reg_resultado)
{
    ...
    while (inicio <= fim)
    {
        ...
        /* Salta diretamente para o byte de início do registro do meio */
...
/* =========================================================================
 * REQUISITO 5: MERGE DE DOIS ARQUIVOS ORDENADOS (SEM CARREGAR TUDO EM MEMÓRIA)
 * ========================================================================= */
int merge_arquivos_ordenados(const char *arq1, const char *arq2, const char *arq_saida)
{
    ...
    /* Intercalação simultânea linear O(N1 + N2) */
    while (tem1 && tem2)
    {
        ...
        {
            /* Se houver chave igual entre arquivos (não esperado pelas premissas do PDF) */
            grava_registro(fout, reg1);
            tem1 = (pega_registro(f1, reg1) > 0);
            if (tem1)
                extrai_cpf(reg1, cpf1);
    ...
    /* Descarrega os registros restantes de f1 */
    while (tem1)
    {
        grava_registro(fout, reg1);
        tem1 = (pega_registro(f1, reg1) > 0);
    ...
    /* Descarrega os registros restantes de f2 */
    while (tem2)
    {
        grava_registro(fout, reg2);
        tem2 = (pega_registro(f2, reg2) > 0);
...
/* =========================================================================
 * AUXILIARES DE ENTRADA PADRÃO (SEM USO DE CONIO.H)
 * ========================================================================= */
...
int main(void)
{
    ...
    do
    {
        ...
        case 3:
        {
            ...
            /* Realiza uma busca prévia para exibir o registro atual */
        ...
        case 4:
        {
            ...
            /* Monta o índice de offsets em memória (Snapshot do arquivo) */
            ...
            /* Libera a memória alocada para o índice */
            free(offsets);
            fclose(f);
            pausa_enter();
            break;
